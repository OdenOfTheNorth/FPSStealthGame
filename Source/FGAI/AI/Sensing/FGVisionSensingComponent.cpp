...
void UFGVisionSensingComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	for (int32 Index = SensedTargets.Num() - 1; Index >= 0; --Index)
	{
		...
		if (!IsPointVisible(Target->GetTargetOrigin(), Origin, Direction, SensingSettings->DistanceMinimum))
		{
			...
			Results.SensedLocation = Target->GetOwner()->GetActorLocation();
			Results.bSensedActor = false;
	...
	{		
		...
		{
			AFGCharacter* Player = Cast<AFGCharacter>(Target->GetOwner());
			if (Player)
			{
				...
				Results.SensedActor = Target->GetOwner();
				Results.SensedLocation = Target->GetOwner()->GetActorLocation();
				Results.bSensedActor = (Target != nullptr);
				OnTargetSensed.Broadcast(Results);
			}
			AFGScreamer* Screamer = Cast<AFGScreamer>(Target->GetOwner());
			if (Screamer != nullptr)
			{
				FName CurrentStateName = Screamer->StateMachine->GetCurrentStateName();
				if (CurrentStateName == "Sus")
				{
					FSensesInfo Results;
					Results = Screamer->TargetingComponent->SensesInfo;
					OnTargetSensed.Broadcast(Results);
				}
			}

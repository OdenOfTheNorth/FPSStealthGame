...
AFGCharacter::AFGCharacter()
{
	...
	VisionTargetComponent = CreateDefaultSubobject<UFGVisionSensingTargetComponent>(TEXT("VisionTargetComponent"));
}
...
{
	FTransform SpawnTransform = FirstPersonCameraComponent->GetComponentTransform();
	SpawnTransform.AddToTranslation(SpawnTransform.GetRotation().GetForwardVector() * 100);
	AImpaleProjectile* Projectile = GetWorld()->SpawnActor<AImpaleProjectile>(ImpaleProjectile, SpawnTransform);
}
